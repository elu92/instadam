# -*- coding: utf-8 -*-
"""
Created on Sat Nov 19 14:15:04 2022
...
class PhotoViewer(QtWidgets.QGraphicsView):
    ...
    def setPhoto(self, pixmap=None):
        ...
        self.fitInView()
        self.viewrect = self.viewport().rect()
        self.zoomed.emit(self._zoom, 1,
                         self.mapToScene(self.viewrect.width() / 2.0, np.round((self.viewrect.height() + 1) / 2.0)))
        ...
        if self.imageSet:
            if self.paintMode:
                if self.viewerType == IM_VIEW:
                    ...
                    self.labels.setPixmap(
                        maskPixmaps(self.labels.pixmap(), self.currentMap, self.imMask, self.brushType))
                else:
